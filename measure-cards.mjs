...
const d = await page.evaluate(() => {
  ...
  return { homeH: Math.round(home), banner: Math.round(banner), portal: Math.round(portal), pc, pm, vh: window.innerHeight };
});
console.log("DESKTOP:", JSON.stringify(d));
await page.screenshot({ path: "D:/新文科比赛/高邮网站/gaoyou-pages/screenshots/small-cards-desktop.png", fullPage: false });
...
const md = await m.evaluate(() => {
  ...
  const pc = [...document.querySelectorAll(".pc-card")].map(c => Math.round(c.getBoundingClientRect().height));
  const pm = [...document.querySelectorAll(".pm-card")].map(c => Math.round(c.getBoundingClientRect().height));
  return { homeH: Math.round(home), portal: Math.round(portal), pc, pm, vh: window.innerHeight };
});
console.log("MOBILE:", JSON.stringify(md));
await m.screenshot({ path: "D:/新文科比赛/高邮网站/gaoyou-pages/screenshots/small-cards-mobile.png", fullPage: false });
await browser.close();
