...
const info = await page.evaluate(() => {
  ...
  return {
    ...
    hasFilterSvg: !!document.querySelector("#brtitle"),
    rect: { x: Math.round(r.x), y: Math.round(r.y), w: Math.round(r.width), h: Math.round(r.height) }
  };
});
console.log("desktop:", JSON.stringify(info, null, 2));
await page.screenshot({ path: "D:/新文科比赛/高邮网站/gaoyou-pages/screenshots/final-title-desktop.png", clip: { x: 0, y: 0, width: 1440, height: 780 } });
...
const minfo = await m.evaluate(() => {
  const h1 = document.querySelector(".hero h1");
  const cs = getComputedStyle(h1);
  return { font: cs.fontFamily.split(",")[0], hasBlurFilter: cs.filter.includes("brtitle"), color: cs.color, hasFilterSvg: !!document.querySelector("#brtitle") };
});
console.log("mobile:", JSON.stringify(minfo, null, 2));
await m.screenshot({ path: "D:/新文科比赛/高邮网站/gaoyou-pages/screenshots/final-title-mobile.png", clip: { x: 0, y: 0, width: 390, height: 844 } });
await browser.close();
