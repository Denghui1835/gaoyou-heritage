...
const m = await page.evaluate(() => {
  ...
  return {
    kicker: h(".hero-kicker"), h1: h(".hero h1"), sub: h(".hero-sub"), btn: h(".hero-btn"),
    pc: [...document.querySelectorAll(".pc-card")].map(c => Math.round(c.getBoundingClientRect().height)),
    pcImg: h(".pc-card .pc-img"), pcBd: h(".pc-card .pc-bd"),
    pm: [...document.querySelectorAll(".pm-card")].map(c => Math.round(c.getBoundingClientRect().height)),
    pmImg: h(".pm-card .pm-img"), pmBd: h(".pm-card .pm-bd"),
    quote: h(".portal-quote")
  };
});
