...
const info = await m.evaluate(() => {
  ...
  const veil = getComputedStyle(document.querySelector(".hero-veil")).backgroundImage;
  return { h1Top: Math.round(h1.top), portalTop: Math.round(portal.top), portalBottom: Math.round(portal.bottom), veilBg: veil };
});
console.log(JSON.stringify(info));
await m.screenshot({ path: "D:/新文科比赛/高邮网站/gaoyou-pages/screenshots/no-veil.png", fullPage: false });
await browser.close();
