...
const d = await page.evaluate(() => {
  ...
  return { heroH: Math.round(hero.height), screenShare: Math.round(hero.height / window.innerHeight * 100) + "%", pc, pm, vh: window.innerHeight };
});
console.log("DESKTOP:", JSON.stringify(d));
await page.screenshot({ path: "D:/新文科比赛/高邮网站/gaoyou-pages/screenshots/smaller-bg-desktop.png", fullPage: false });
...
const md = await m.evaluate(() => {
  const hero = document.querySelector(".hero").getBoundingClientRect();
  return { heroH: Math.round(hero.height), screenShare: Math.round(hero.height / window.innerHeight * 100) + "%", vh: window.innerHeight };
});
console.log("MOBILE:", JSON.stringify(md));
await m.screenshot({ path: "D:/新文科比赛/高邮网站/gaoyou-pages/screenshots/smaller-bg-mobile.png", fullPage: false });
await browser.close();
